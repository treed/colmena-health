    ...
    let
      ...
      addTag =
        ...
        (
          ...
          assert
            !(module ? option.type)
            || abort "Module passed to oneOfTagged canno't have an option called type (reserved for typetag)";
          imports // config // { options = module.options // tagModule; }
        );
      ...
      oneOfTagged = definitions: types.oneOf (mapAttrDefs definitions);
